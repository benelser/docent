// `objection` scene component — migrated from
// packages/engine/src/scenes/ObjectionScene.tsx.
//
// Behavior is unchanged from v2.5.x: same DOM, same animation curves, same
...
// reading the same fields through the new shape.
//
 ...
 * `Scene`.
 *
 * `kicker` and `heading` are common scene fields read by this component
 ...
 * access.
 */
export interface ObjectionSpec extends Scene {
  type: 'objection';
  claim?: string;
  objection?: string;
  evidence?: string[];
...
export const ObjectionSceneComponent: React.FC<SceneRenderProps<ObjectionSpec>> = ({
  scene,
  common,
}) => {
...
export default ObjectionSceneComponent;
