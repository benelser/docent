// Local Narration — lays each beat's narration audio on the scene
// timeline.
//
// MIRROR of packages/engine/src/components/Narration.tsx, adapted to the
// kit's BeatTimelineSlot shape (the v2.5.x engine's TimedBeat surfaced
// `from`/`durationInFrames`/`audio` directly; the kit's BeatTimelineSlot
// exposes `startFrame`/`frames` with the beat itself nested under
// `beat`, where plugin-owned fields like `audio` live in the open index
// signature). When the audio-rhythm/narration FeaturePlugin lands in
// @docent/core and the audio pipeline is reconciled, this file goes away
// in favour of a shared import.
