...
//   - props receive `SceneRenderProps<CausalLoopScene>` from @bjelser/kit
...
//     glow, activeBeatIndex, the cadence + palette knob helpers, the SVG
//     text fitters) lives as colocated helpers in this scene's directory
//     until the shared-infra migration agent lands; the integrator will
//     swap the underscore-prefixed local helpers for shared imports at
//     merge time.
//
...
// — even count of '-' edges, the cycle compounds) or B (balancing — odd
// count of '-' edges, the cycle self-corrects). The validator enforces the
// labelling math; this renderer reads the spec's `kind` and shows it.
//
// The argument the scene makes IS the cycle. Reveal beats animate
// variables → edges → the loop label, so the viewer sees the loop close
// before the glyph that names what it does. A causal loop is fundamentally
// STATIC: the edges argue *what causes what*, not data flowing through
// them. There are no pulses; the polarity glyph carries the assertion.
...
import type {
  CausalEdge,
  CausalLoop,
  CausalLoopScene as CausalLoopSceneSpec,
  CausalVariable,
} from './validate';
...
// The point on the unit circle from `from` → `to`, offset inward by
// ARROW_GAP from each disc's edge. Used so the arrow tip kisses the disc
// without burying into it.
const edgePoints = (
...
export const CausalLoopSceneComponent: React.FC<
  SceneRenderProps<CausalLoopSceneSpec>
> = ({scene, common}) => {
  ...
  ts.beats.forEach((b) => {
    ...
    if (Array.isArray(reveal)) {
      (reveal as string[]).forEach((id, order) => {
        if (revealFrame[id] === undefined) {
          revealFrame[id] = b.startFrame + cadenceOffset(b.beat.cadence, order);
          revealCadence[id] = b.beat.cadence;
        }
      });
    }
  });
  ...
  const focusIds = new Set<string>(
    Array.isArray(focusRaw) ? (focusRaw as string[]) : [],
  );
      ...
      <AbsoluteFill>
        ...
        >
          ...
          {edges.map((e) => {
            ...
            // (mid) to the end.
            const angle =
              (Math.atan2(end.y - mid.y, end.x - mid.x) * 180) / Math.PI;
            ...
            return (
              <g key={e.id} opacity={opacity}>
                ...
                <path
                  ...
                  opacity={0.85}
                  style={{
                    filter: `drop-shadow(0 0 6px ${glow(accentHex, 0.45)})`,
                  }}
                />
                {/* arrowhead */}
                <g
                ...
                {e.label
                  ? (() => {
                      const fs = fitFontSize(e.label, {
                        maxWidth: 240,
                        basePx: 14,
                        floorPx: 10,
                        charAdvance: 0.6,
                      });
                      const txt = truncateForSlot(e.label, {
                        maxWidth: 240,
                        fontSize: fs,
                        charAdvance: 0.6,
                      });
          ...
          {/* ----- loop centre labels — R (reinforcing) / B (balancing) ----
              ...
              indicator arrow under it (the "loop" arc) so the label reads
              as a *loop*, not as decoration. */}
          {loops.map((loop) => {
            ...
            return (
              <g key={loop.id} opacity={draw}>
                ...
                <circle
                  ...
                  opacity={0.95}
                  style={{
                    filter: `drop-shadow(0 0 18px ${glow(labelHex, 0.6)})`,
                  }}
                />
                <text
                  x={cx}
                ...
                {loop.label
                  ? (() => {
                      const fs = fitFontSize(loop.label, {
                        maxWidth: 280,
                        basePx: 15,
                        floorPx: 10,
                        charAdvance: 0.6,
                      });
                      const txt = truncateForSlot(loop.label, {
                        maxWidth: 280,
                        fontSize: fs,
                        charAdvance: 0.6,
                      });
