// The camera. A scene's diagram lives in a fixed 1920x1080 "world"; the
// camera leans toward the component under discussion. Crucially it is
// *hard-clamped*: the diagram can never ride up under the heading or off any
// edge — the camera pans and zooms only within a safe band, so it cannot
// break the layout.
//
// Mirrors packages/engine/src/engine/camera.ts exactly, adapted to walk the
// kit's BeatTimelineSlot[] (the v2.5.x engine read `from`/`focus`/`shot` off
// TimedBeat directly; the kit's beat-shaped fields live under
// `slot.beat.{focus,shot}` with `slot.startFrame` as the start-of-beat
// frame).
...
export const resolveCamera = (
  ...
  const readShot = (i: number): Shot | undefined => {
    const v = (beats[i]?.beat as {shot?: Shot} | undefined)?.shot;
    return v;
  };
  const readFocus = (i: number): string[] => {
    const v = (beats[i]?.beat as {focus?: readonly string[]} | undefined)?.focus;
    return Array.isArray(v) ? [...v] : [];
  };
  const shotFor = (i: number): CameraState => {
    const shot: Shot = readShot(i) ?? 'follow';
    const f = readFocus(i)
      .map((id) => boxesById[id])
  ...
  const held = readShot(active) === 'hold';
  const cur = held ? prev : shotFor(active);
  const local = frame - (beats[active]?.startFrame ?? 0);
  const p =
