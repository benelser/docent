...
// only the resting visual state. The structure scene's nodes may carry an
// embed (allowlist: mechanism | chart | venn), and this module is the renderer.
//
// MIRROR of packages/engine/src/scenes/EmbeddedScene.tsx, narrowed to the
// embed types a structure host can carry (the structure allowlist is
// mechanism/chart/venn; we keep the other branches for parity with the
// engine because validate.ts owns the allowlist enforcement — the renderer
// just falls back to `null` for an out-of-allowlist embed type).
...
// The embed body — an open shape mirroring the structure scene's host slot.
// The renderer reads whichever fields the embed type carries.
type EmbedSpec = {
  type: string;
  caption?: string;
  // mechanism
  ...
  figures?: Array<{id: string; label: string; value: string | number; unit?: string}>;
  // compare
  ...
  // tree
  root?: {id: string; label: string; children?: Array<unknown>};
  [k: string]: unknown;
};
      ...
      {series.map((s, si) => {
        ...
        if (s.kind === 'bars' && Array.isArray(s.data)) {
          ...
          return (
            <g key={`b-${si}`}>
              {data.map((d, di) => {
                const bx = x0 + (di + 0.5) * (w / data.length) - barW / 2;
...
const renderTreeEmbed = (
  ...
  const bg = style.tokens.bg;
  const monoFamily = style.tokens.typography.family.mono;
  const levels: TreeLvl[][] = [];
  const all: TreeLvl[] = [];
  const walk = (n: {id: string; label: string; children?: Array<unknown>}, depth: number, parent: string | null): void => {
    if (!levels[depth]) levels[depth] = [];
    const entry: TreeLvl = {id: n.id, label: n.label, depth, parent, xFrac: 0};
    levels[depth].push(entry);
    all.push(entry);
    if (Array.isArray(n.children)) {
      n.children.forEach((c) =>
        walk(c as {id: string; label: string; children?: Array<unknown>}, depth + 1, n.id),
      );
    }
  };
  walk(root, 0, null);
  ...
  const posOf = (n: TreeLvl): {x: number; y: number} => ({
