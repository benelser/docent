// Inlined helpers for the recap scene.
//
// Mirrors the v2.5.x engine's shared `glow` utility and the
// `activeBeatIndex` walker — adapted to the kit's BeatTimelineSlot shape
// (which exposes `startFrame` instead of the legacy `from`). The v3.0
// fan-out moves each scene into its own directory; the shared component
// infrastructure (SceneFrame, Narration, FittedText, fonts, glow,
// activeBeatIndex) will be migrated by separate agents and reconciled by
// the integrator at merge time. For now we colocate the minimum each
// scene needs so the per-scene worktree builds clean.
//
// When the shared-infra migration lands, the recap scene will import
