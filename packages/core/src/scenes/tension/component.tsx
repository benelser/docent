...
//     Node, FittedText, Narration, fonts) lives as colocated helpers in this
//     scene's directory until the shared-infra migration agent lands; the
//     integrator will swap the underscore-prefixed local helpers for shared
//     imports at merge time.
//
...
// below. Authors declare `kind: 'rejected'` and `kind: 'risk'`; the renderer
// maps `kind` onto the ledger lane, so the spec contract is unchanged — the
// grid `col`/`row` is ignored here (the ledger owns placement).
//
// Two registers, both built from the same primitives so they read as one
// family with two voices rather than two scenes:
//
//   sketch (default) — the dark register. Sits inside the same parallax
//     space, dotted grid, vignette, kicker, and progress chrome as every
//     other docent scene; the trade-off itself is the diagram.
//
//   whiteboard (the light register, retired in v2.4.0 with the `treatment`
//     knob — preserved in the renderer in case a future caller wires it
//     back). A cream paper backdrop, deep marker-blue ink, drop-shadow card
//     facets. The chrome is rebuilt locally with the same vocabulary on
//     paper.
//
...
export const TensionSceneComponent: React.FC<
  SceneRenderProps<TensionSceneSpec>
> = ({scene, common}) => {
  const frame = useCurrentFrame();
  ...
  // Coerce the spec's tension-typed nodes into the layout-helper Node shape.
  // The two shapes are structurally compatible (id/label/sub/kind/col/row/…)
  // — the helper's `col` and `row` are required-number where the spec's are
  // optional, so we backfill with 0 before the resolver walks them.
  const specNodes = Array.isArray(scene.nodes) ? scene.nodes : [];
  const layoutNodes: Node[] = specNodes.map((n) => ({
    id: n.id,
    label: n.label,
    sub: n.sub,
    tag: n.tag,
  ...
  ts.beats.forEach((b) => {
    ...
    if (Array.isArray(reveal)) {
      reveal.forEach((id) => {
        if (typeof id === 'string' && revealFrame[id] === undefined) {
          revealFrame[id] = b.startFrame;
        }
      });
    }
  });
  ...
  const activeBeat = ts.beats[active]?.beat as {focus?: unknown} | undefined;
  const focusList = Array.isArray(activeBeat?.focus) ? activeBeat?.focus : [];
  const focusIds = new Set<string>(
    (focusList as unknown[]).filter((v): v is string => typeof v === 'string'),
  );
  ...
  // The Narration expects ResolvedStyle; cast through to silence any
  // structural mismatch when a future build augments the type.
  const resolvedStyle: ResolvedStyle = style;
  ...
  return (
    ...
    >
      ...
      <Narration style={resolvedStyle} beats={ts.beats} />
    </AbsoluteFill>
  );
};
