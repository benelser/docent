...
// where plugin-owned fields like `audio` live in the open index signature).
// When the audio-rhythm/narration FeaturePlugin lands in @docent/core and
// the audio pipeline is reconciled, this file goes away in favour of a
// shared import.
  ...
  return (
    <>
      {beats.map((b) => {
        ...
        if (!src) return null;
        const key =
          (typeof b.beat.id === 'string' && b.beat.id) || `beat-${b.beatIndex}`;
        return (
          <Sequence
            key={key}
            from={b.startFrame}
