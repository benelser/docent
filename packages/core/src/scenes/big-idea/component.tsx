...
//     glow) lives as colocated helpers in this scene's directory until
//     the shared-infra migration agent lands; the integrator will swap
//     the underscore-prefixed local helpers for shared imports at merge
//     time.
//
...
// pace, positioned immediately before the recap); validate.ts and
// depth-rules.ts enforce it.
//
...
// knob). The backdrop branches for `WhiteboardBackdrop` and
// `SketchBackdrop` stay so a future style-driven re-introduction can flip
// them back on without restructuring the scene.
...
export const BigIdeaSceneComponent: React.FC<SceneRenderProps<BigIdeaSceneSpec>> = ({
  scene,
  common,
}) => {
