// ProgressionScene — an ordered timeline track: stages laid along a path,
// each a marker with a label, sub, and the duration of its segment. A
// `gate` stage is preceded by a milestone diamond. Stages appear one beat
// at a time.
//
...
//   - props receive `SceneRenderProps<ProgressionScene>` from @bjelser/kit
...
//     glow, activeBeatIndex, cadence + palette helpers) lives as
//     colocated helpers in this scene's directory until the shared-infra
//     migration agent lands; the integrator will swap the underscore-
//     prefixed local helpers for shared imports at merge time.
//   - the kit's `BeatTimelineSlot` exposes `{startFrame, frames, beat,
//     beatIndex}`; the v2.5.x engine's `TimedBeat` exposed `{from,
//     durationInFrames, reveal, cadence, focus, audio}` flatly. The
//     component reads `from` ← `startFrame` and pulls plugin-owned beat
//     fields (`reveal`, `cadence`, `focus`) off `slot.beat`.
//
...
// list as plugin-owned fields under its open index signature. The reader
// helpers below pull them off without widening the engine's contract.
const beatReveal = (b: Beat): number | undefined => {
  const v = b.reveal;
  return typeof v === 'number' ? v : undefined;
};

const beatFocus = (b: Beat): ReadonlyArray<string> => {
  const v = (b as {focus?: unknown}).focus;
  return Array.isArray(v) ? (v.filter((x) => typeof x === 'string') as string[]) : [];
};

// Adapter: turn the kit's BeatTimelineSlot[] into the {from, reveal,
// cadence}[] shape numericRevealMap reads (mirrors the v2.5.x engine
// signature exactly — see _helpers.ts).
const beatsForReveal = (
  beats: ReadonlyArray<BeatTimelineSlot>,
): Array<{from: number; reveal?: number | undefined; cadence?: Beat['cadence']}> =>
  beats.map((b) => ({
    from: b.startFrame,
...
export const ProgressionSceneComponent: React.FC<SceneRenderProps<ProgressionSceneSpec>> = ({
  scene,
  common,
}) => {
  ...
  // resolves through the resolved-style accent table.
  const accentHex = paletteSceneHex(undefined, undefined, style);
  const stages: ProgressionStage[] = scene.stages ?? [];
  ...
  const reveals = numericRevealMap(beatsForReveal(ts.beats), stages.length);
  ...
  const activeBeat = ts.beats[active]?.beat;
  const focusIds = new Set(activeBeat ? beatFocus(activeBeat) : []);
  ...
  const laneGrow = (track: 0 | 1): number => {
    ...
    const lastGi = idx[lastInLane];
    if (lastGi === undefined) return 0;
    const local = frame - revealFrameFor(lastGi);
    const p = local <= 0 ? 0 : spring({frame: local, fps, config: {damping: 200}});
          ...
          {braided ? (
            <>
              ...
              {stages.map((s, i) => {
                ...
                if (!partner || s.track === partner.track) return null;
