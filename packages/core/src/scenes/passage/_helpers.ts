// Inlined helpers for the passage scene.
//
// These mirror the v2.5.x engine's shared `glow` utility and
// `activeBeatIndex`. The v3.0 fan-out moves each scene into its own
// directory in @docent/core; the shared component infrastructure
// (SceneFrame, Narration, FittedText, fonts, glow) will be migrated by
// separate agents and reconciled by the integrator at merge time. For
// now we colocate the minimum each scene needs so the per-scene
// worktree builds clean.
//
// When the shared-infra migration lands, the passage scene will import
