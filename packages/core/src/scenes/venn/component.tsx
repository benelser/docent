...
//   - props receive `SceneRenderProps<VennSceneSpec>` from @bjelser/kit
...
//     glow, activeBeatIndex, palette-resolvers) lives as colocated helpers
//     in this scene's directory until the shared-infra migration agent
...
// Resolve the scene's accent hex from the active style's token table. The
// v2.5.x engine routed this through `paletteSceneHex(undefined, undefined,
// style)` — with no palette set (the v2.4.0+ default state of every caller)
// the resolution is exactly `style.tokens.accent.blue ?? '#5cb6ff'`.
const accentOf = (style: ResolvedStyle): string => {
  const map = style.tokens.accent as unknown as Record<string, string | undefined>;
  return map.blue ?? '#5cb6ff';
};

// Reads from beat fields that aren't in the kit's typed shape (reveal is in
// the kit, focus is plugin-owned and lives in the open index signature).
const beatReveal = (beat: unknown): readonly string[] | undefined => {
  const v = (beat as {reveal?: unknown}).reveal;
  return Array.isArray(v) ? (v as readonly string[]) : undefined;
};
const beatFocus = (beat: unknown): readonly string[] | undefined => {
  const v = (beat as {focus?: unknown}).focus;
  return Array.isArray(v) ? (v as readonly string[]) : undefined;
};

export const VennSceneComponent: React.FC<SceneRenderProps<VennSceneSpec>> = ({
  scene,
  common,
}) => {
  ...
  for (let i = 0; i <= active; i++) {
    const r = beatReveal(ts.beats[i]?.beat);
  ...
  const focusIds = new Set(beatFocus(ts.beats[active]?.beat) ?? []);
  const hasFocus = focusIds.size > 0;

  const enterFrameFor = (id: string): number => {
    for (const b of ts.beats) {
      const r = beatReveal(b.beat);
      if (Array.isArray(r) && r.includes(id)) return b.startFrame;
    }
    return 0;
  };
  ...
  const setEnterAlpha = (sid: string): number => {
    if (allByDefault || !ts.beats.some((b) => {
      const r = beatReveal(b.beat);
      return Array.isArray(r) && r.some((id) => setIdToIndex.has(id));
    })) {
      return intro;
    }
  ...
  const circleTintFor = (_i: number): string => {
  ...
  // Void the imports if not used yet (kept available so a future swap to a
  // shared-infra import is one line — the interFamily/monoFamily families
  // are used inside FittedText style props below).
  void interFamily;
...
export default VennSceneComponent;
