// EmbeddedScene — Sprint B compositional grammar.
//
// CANONICAL renderer for one of the eight embed types
// (mechanism | venn | chart | quantities | compare | structure |
// causal-loop | tree), drawn as a STATIC tableau inside a host scene's
// allocated bounds. No animation, no audio, no beats: only the resting
// visual state. The host owns the timing (when the embed appears, when
// it dims) via reveal/focus on the slot's id; this component owns the
// pixels.
//
// Bounds are in stage (1920×1080) coordinates so the host can place the
// embed inside the parent's SVG/AbsoluteFill without an extra transform.
// The inherited style flows through paletteSceneHex / the ink+bg tokens.
//
...
//
// Why one component instead of six embedded-mode flags on the host
// renderers: the host renderers carry beat-orchestration, narration
// overlays, camera, and full-stage layouts — none of which apply inside
// a slot. A single tableau renderer per embed type keeps the change
// surface small and the static-only contract impossible to violate by
// accident.
...
// The embed payload — a permissive shape mirroring the v2.5.x engine's
// EmbeddedScene spec. The per-type renderers narrow to the fields they
// actually read; the kit treats `embed` as opaque otherwise. Keeping the
// shape open avoids locking the embed schema in this file (the
// per-scene-type schemas / validators own that).
export interface EmbeddedSceneSpec {
  type: string;
  id?: string;
  caption?: string;
  // mechanism
  parts?: Array<{id: string; label: string; pos: {x: number; y: number}}>;
  motion?: {kind: string; path?: string[]; [k: string]: unknown};
  // venn
  ...
  rows?: Array<{
    id: string;
    label: string;
    cells?: Array<{text?: string; verdict?: 'win' | 'lose' | 'neutral'}>;
  }>;
  // structure
  nodes?: Array<{
    id: string;
  ...
  causalEdges?: Array<{id: string; from: string; to: string}>;
  // tree (recursive)
  root?: TreeNode;
  [k: string]: unknown;
}
...
// drawing the resting state of the scene type. Long text routes through
      ...
      {motion?.kind === 'cycle' && Array.isArray(motion.path) ? (
        <g opacity={0.85}>
          {motion.path.map((id, i) => {
            ...
            const to = parts.find((p) => p.id === path[(i + 1) % path.length]);
        ...
        if (s.kind === 'bars' && Array.isArray(s.data)) {
          ...
          return (
            <g key={`b-${si}`}>
              {data.map((d, di) => {
                const bx =
                  x0 + (di + 0.5) * (w / data.length) - barW / 2;
...
const renderCausalLoopEmbed = (
  ...
  vars.forEach((v, i) => {
    const theta = (i / Math.max(1, vars.length)) * Math.PI * 2 - Math.PI / 2;
    positions.set(v.id, {
...
const renderTreeEmbed = (
  ...
  const levels: Lvl[][] = [];
  const all: Lvl[] = [];
  const walk = (n: TreeNode, depth: number, parent: string | null): void => {
  ...
  return (
    <g>
      ...
      {all.map((n) => {
        ...
        const fs = fitFontSize(n.label, {
          maxWidth:
            (bounds.w / Math.max(1, (levels[n.depth] || []).length)) * 0.85,
...
export const EmbeddedScene: React.FC<Props> = ({
  ...
  switch (embed.type) {
    ...
    case 'tree':
      ...
      break;
    default:
      // Out-of-allowlist or unknown types fall through to outline + caption
      // only. Per-host validators are the source of truth for the allowlist;
      // the renderer degrades gracefully so a half-written spec in studio
      // still shows the bounds.
      body = null;
  }
