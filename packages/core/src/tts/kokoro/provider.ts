...
class KokoroProvider implements TtsProvider {
  ...
  async listVoices(): Promise<TtsVoice[]> {
    ...
    return Object.entries(voices).map(([id, meta]) => {
      ...
      if (meta.traits) {
        voice.metadata = {traits: meta.traits};
      }
      return voice;
    });
  }
...
/**
 * Construct a live Kokoro TTS provider instance from the engine-supplied
 * context. `create()` is cheap — the heavy ONNX model load is deferred to
 * the first `synth()` call (matches the design doc's "lazy by default"
 * policy, Open Question O8).
 */
export const createKokoroProvider = async (
