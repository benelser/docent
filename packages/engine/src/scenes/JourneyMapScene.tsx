...
// A continuous emotional curve runs across the top: high = good emotion,
// low = bad. Reveal beats walk one stage at a time; focused stages glow.
//
// The shape is intentionally different from `progression` (system-internal
// stages over time) and `walkthrough` (message-passing). The spine here is
// *a person's emotional arc* anchored to the stages they walk through —
// what they feel, what they touched, what hurt.

// The closed allowlist of journey emotions, paired with a colour and a short
...
export const JourneyMapScene: React.FC<SceneProps & {style: ResolvedStyle}> = ({
  ...
  // reveal map gives, per stage index, the revealing beat's frame, cadence,
  // and within-batch order. A knob-free scene is byte-identical with the
  // pre-cadence behaviour.
  ...
  // The journey band's geometry. Stages sit evenly along a horizontal axis;
  // the emotion curve runs in a band above them, the chips ride on the axis,
  // the touchpoint cards stack below.
  const left = 220;
  const right = 1700;
  const axisY = 580;
    ...
    >
      <AbsoluteFill>
        ...
        >
          ...
          {/* dots along the curve, one per revealed stage — they let the eye
              pin a stage's specific feeling to a point on the arc */}
