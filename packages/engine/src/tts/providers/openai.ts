...
const OPENAI_CAPABILITIES: TtsCapabilities = {
...
class OpenAIProvider implements TtsProvider {
  ...
  async synth(text: string, options: TtsSynthesisOptions): Promise<TtsSynthesisResult> {
    ...
    const responseFormat = options.format === 'pcm' ? 'pcm' : options.format === 'wav' ? 'wav' : 'mp3';
...
export const openaiProvider: TtsProviderPlugin = {
  providerId: 'openai',
  version: '1.0.0',
