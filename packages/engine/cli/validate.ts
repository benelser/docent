...
const SCENE_TYPES = ['frame', 'structure', 'progression', 'walkthrough', 'compare', 'quantities', 'probe', 'tension', 'closeup', 'passage', 'figure', 'demonstrate', 'recap', 'diff', 'chart', 'big-idea', 'prior-art', 'causal-loop'];
...
export const validateSpec = (spec: unknown): ValidationIssue[] => {
  ...
  s.scenes.forEach((sc: Record<string, any>, i: number) => {
    ...
    if (sc.type === 'causal-loop') {
      ...
      // causalEdges — directed edges with a polarity glyph. Each one MUST
      // reference real variables and carry an exact '+' or '-'. We build a
      // (from,to) → polarity map for the loop-parity check below; only edges
      // whose endpoints are valid populate it (a broken edge is reported
      // separately).
      ...
      // loops — every loop's `path` must reference real variables, close
      // (each consecutive pair must have an edge, and the last → first must
      // have one too), and its `kind` must match the parity of '-' edges
      // along the path. R = even '-' count; B = odd '-' count. The
      // depthcheck enforces the closure dimension separately (a loop that
      // never closes is a line, not a loop).
      ...
      if (sc.loops !== undefined && !Array.isArray(sc.loops)) {
        ...
        sc.loops.forEach((loop: Record<string, any>, k: number) => {
          ...
          // Validate each variable id in the path resolves to a real variable.
          let pathOk = true;
          ...
          if (!pathOk) return;
          // Each consecutive pair (and the wrap last→first) must have a
          // causal edge connecting them. A loop drawn over a non-existent
          // edge fails the spec — the engine cannot animate a line that
          // does not exist.
          let minusCount = 0;
          ...
          if (!pathOk) return;
          // The HARD-FAIL: the kind must be parity-matched against the
          // '-' count. This is the line between a labelled diagram and a
          // diagram that argues — if R/B can lie, the scene says nothing.
      ...
      // A non-causal-loop scene must not carry causal-loop fields — they
      // would silently render nothing on the wrong scene type.
