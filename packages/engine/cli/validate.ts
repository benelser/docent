...
const SCENE_TYPES = ['frame', 'structure', 'progression', 'walkthrough', 'compare', 'quantities', 'probe', 'tension', 'closeup', 'passage', 'figure', 'demonstrate', 'recap', 'diff', 'chart', 'big-idea', 'prior-art', 'map', 'timeline', 'tree'];
...
export const validateSpec = (spec: unknown): ValidationIssue[] => {
  ...
  s.scenes.forEach((sc: Record<string, any>, i: number) => {
    ...
    if (sc.type === 'map') {
      ...
      // map-specific fields have no meaning on other scene types.
      ...
      if (sc.regions !== undefined) {
        issues.push({path: `${at}.regions`, message: `regions has no meaning for type "${sc.type}" — only map`});
      }
