...
class ElevenLabsProvider implements TtsProvider {
  ...
  async synth(text: string, options: TtsSynthesisOptions): Promise<TtsSynthesisResult> {
    ...
    const last = alignment.length > 0 ? alignment[alignment.length - 1] : undefined;
    const durationMs = last ? last.endMs : 0;
  ...
  async listVoices(): Promise<TtsVoice[]> {
    ...
    try {
      ...
      return voices.map((v) => {
        ...
        if (v.labels?.gender) out.gender = v.labels.gender;
        if (v.labels) out.metadata = v.labels;
        return out;
      });
    } catch (e) {
      throw new TtsProviderError(
...
/**
 * Construct a live ElevenLabs TTS provider instance. Throws
 * `TtsProviderError` synchronously if `ELEVENLABS_API_KEY` is not present.
 */
export const createElevenLabsProvider = async (
